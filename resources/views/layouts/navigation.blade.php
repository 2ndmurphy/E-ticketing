@php
		...
		if (Auth::check()) {
		    $role = Auth::user()->role;
		    $menus = MenuHelper::getMenuByRole($role);
		    $profileMenu = MenuHelper::findProfileMenu($menus);
		}
@endphp
				...
				<div class="flex h-16 justify-between">
								...
								@auth
										<div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
												@foreach ($menus as $item)
														...
														@continue(str_contains($item['route'] ?? '', 'profile')) {{-- Skip menu profil --}}
						...
						<div class="hidden sm:ms-6 sm:flex sm:items-center">
								<x-dropdown align="right" width="48">
										...
										<x-slot name="content">
												@if ($profileMenu)
														<x-dropdown-link :href="route($profileMenu['route'])">
																{{ $profileMenu['label'] }}
														</x-dropdown-link>
												@endif
		...
		<div class="hidden sm:hidden" :class="{ 'block': open, 'hidden': !open }">
				...
				<div class="border-t border-gray-200 pb-1 pt-4 dark:border-gray-600">
						...
						<div class="mt-3 space-y-1">
								@if ($profileMenu)
										<x-responsive-nav-link :href="route($profileMenu['route'])">
												Profile
										</x-responsive-nav-link>
								@endif
